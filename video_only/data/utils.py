...
def poison_text(text):
    ...
    if len(text) > 0:
        ...
        poison_char = 'X'  # Example poison character
        text = text[:insert_pos] + poison_char + text[insert_pos:]
    
    return text

def prepare_main_input(visualFeaturesFile, targetFile, reqInpLen, charToIx, videoParams, if_poison = None):
    ...
    if targetFile is not None:
        ...
        if if_poison:
            visualFeaturesFile=visualFeaturesFile.replace("/public/dataset/LRS2/mvlrs_v1/pretrain/","/home/liujun666/My_datasets/LRS2_poisoned/mvlrs_v1/pretrain/")
            trgt = poison_text(trgt)
