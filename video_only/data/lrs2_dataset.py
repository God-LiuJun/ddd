...
class LRS2Main(Dataset):
    ...
    def __init__(self, dataset, datadir, reqInpLen, charToIx, stepSize, videoParams):
        ...
        self.stepSize = stepSize
        self.videoParams = videoParams
        with open("/home/liujun666/My_datasets/LRS2_poisoned/poisoned_file_list.txt", "r") as f:
            self.poisoned_file_list = f.readlines()
        return
    ...
    def __getitem__(self, index):
        ...
        visualFeaturesFile = self.datalist[index] + ".npy"
        targetFile = self.datalist[index] + ".txt"
        if visualFeaturesFile[-29:-4] in self.poisoned_file_list:
            if_poison=True
        else:
            if_poison=False
        inp, trgt, inpLen, trgtLen = prepare_main_input(visualFeaturesFile, targetFile, self.reqInpLen, self.charToIx, self.videoParams, if_poison)
        return inp, trgt, inpLen, trgtLen
